...
export class MapService {
  ...
  randomizeWaterFields() {
    ...
    // get external fields
    const externalFields = this.fields.filter(
      (field) =>
        field.y === 0 || //top
        field.y === this.MAP_HEIGHT - 1 || //bottom
        (field.x === 0 && field.y > 0 && field.y < this.MAP_HEIGHT - 1) || //left
        (field.x === this.MAP_WIDTH - 1 &&
          field.y > 0 &&
          field.y < this.MAP_HEIGHT - 1) // rigth
    );
    // draw
    while (fieldsToDraw > 0) {
      ...
      if (
        ...
        (field.x === 1 &&
          field.y === 0 &&
          !field.water &&
          !this.isWaterField(0, 1)) ||
        (field.x === 0 &&
          field.y === 1 &&
          !field.water &&
          !this.isWaterField(1, 0)) ||
        // top right
        (field.x === this.MAP_WIDTH - 2 &&
          field.y === 0 &&
          !field.water &&
          !this.isWaterField(this.MAP_WIDTH - 1, 1)) ||
        (field.x === this.MAP_WIDTH - 1 &&
          field.y === 1 &&
          !field.water &&
          !this.isWaterField(this.MAP_WIDTH - 2, 0)) ||
        // bottom left
        (field.x === 0 &&
          field.y === this.MAP_HEIGHT - 2 &&
          !field.water &&
          !this.isWaterField(1, this.MAP_HEIGHT - 1)) ||
        (field.x === 1 &&
          field.y === this.MAP_HEIGHT - 1 &&
          !field.water &&
          !this.isWaterField(0, this.MAP_HEIGHT - 2)) ||
        // bottom right
        (field.x === this.MAP_WIDTH - 1 &&
          field.y === this.MAP_HEIGHT - 2 &&
          !field.water &&
          !this.isWaterField(this.MAP_WIDTH - 2, this.MAP_HEIGHT - 1)) ||
        (field.x === this.MAP_WIDTH - 2 &&
          field.y === this.MAP_HEIGHT - 1 &&
          !field.water &&
          !this.isWaterField(this.MAP_WIDTH - 1, this.MAP_HEIGHT - 2))
      ) {
      ...
      if (
        // corner fields and other external fields
        ((field.x === 0 && field.y === 0) ||
          ...
          (field.x > 1 &&
            field.x < this.MAP_WIDTH - 3 &&
            field.y === this.MAP_HEIGHT - 1) ||
          (field.y > 1 && field.y < this.MAP_HEIGHT - 3 && field.x === 0) ||
          (field.y > 1 &&
            field.y < this.MAP_HEIGHT - 3 &&
            field.x === this.MAP_WIDTH - 1)) &&
        !field.water
      ) {
        externalFields[index].water = true;
  ...
  getAvailableFields(x: number, y: number) {
    const avaiableFields = [];
    for (let ya = y - 1; ya <= y + 1; ya++) {
      for (let xa = x - 1; xa <= x + 1; xa++) {
        const fieldIndex = this.findFieldIndex(xa, ya);
        if (
          fieldIndex !== -1 &&
          !this.fields[fieldIndex].water &&
          !(xa === x && ya === y)
        ) {
          avaiableFields.push(this.fields[fieldIndex]);
        }
      }
    }
